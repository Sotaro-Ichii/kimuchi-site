...
function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#18181b] via-[#232326] to-[#18181b] flex flex-col items-center px-4 py-8">
      {/* ヒーローセクション */}
      <section className="w-full max-w-2xl flex flex-col items-center text-center mb-12 mx-auto">
      ...
      {/* ご利用条件カード（横並び/スマホは縦） */}
      <section className="w-full max-w-4xl flex flex-col gap-8 items-center mb-12">
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full justify-center">
          <div className="flex-1 bg-[#232326] rounded-xl shadow-lg border border-[#27272a] p-6 flex flex-col items-center text-center">
            <FaCheckCircle className="text-[#22d3ee] text-3xl mb-2" />
            <div className="font-semibold text-lg mb-1">承認制</div>
            <div className="text-[#e4e4e7] text-sm">GoogleまたはEmailログイン＋申請必須</div>
          </div>
          <div className="flex-1 bg-[#232326] rounded-xl shadow-lg border border-[#27272a] p-6 flex flex-col items-center text-center">
            <FaMoneyCheckAlt className="text-[#fbbf24] text-3xl mb-2" />
            <div className="font-semibold text-lg mb-1">Zelleで一括支払い</div>
            <div className="text-[#e4e4e7] text-sm">$49.9 / 3ヶ月</div>
          </div>
          <div className="flex-1 bg-[#232326] rounded-xl shadow-lg border border-[#27272a] p-6 flex flex-col items-center text-center">
            <FaLock className="text-[#c92a2a] text-3xl mb-2" />
            <div className="font-semibold text-lg mb-1">外部共有は厳禁</div>
            <div className="text-[#e4e4e7] text-sm">ログイン情報・内容の外部共有は<strong>厳禁</strong></div>
          </div>
        </div>
      </section>

      {/* ご利用までの流れ（ステップバー風） */}
      <section id="apply" className="w-full max-w-3xl bg-[#232326] rounded-2xl shadow-xl border border-[#27272a] p-8 mb-10 animate-fade-in flex flex-col items-center">
        <h3 className="text-xl font-bold text-[#fbbf24] mb-6 flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row gap-6 w-full justify-center items-center mb-6">
          <div className="flex-1 flex flex-col items-center">
            <div className="bg-[#18181b] border border-[#22d3ee] rounded-full w-12 h-12 flex items-center justify-center text-2xl font-bold text-[#22d3ee] mb-2">1</div>
            <div className="text-[#e4e4e7] text-center text-sm">Zelleで <strong>$49.9</strong> を一括zell送金<br /><span className="text-xs">（送金先電話番号: 657-709-1289）</span></div>
          </div>
          <div className="hidden md:block text-3xl text-[#6366f1]">→</div>
          <div className="flex-1 flex flex-col items-center">
            <div className="bg-[#18181b] border border-[#22d3ee] rounded-full w-12 h-12 flex items-center justify-center text-2xl font-bold text-[#22d3ee] mb-2">2</div>
            <div className="text-[#e4e4e7] text-center text-sm">申請フォームに必要事項を記入</div>
          </div>
          <div className="hidden md:block text-3xl text-[#6366f1]">→</div>
          <div className="flex-1 flex flex-col items-center">
            <div className="bg-[#18181b] border border-[#22d3ee] rounded-full w-12 h-12 flex items-center justify-center text-2xl font-bold text-[#22d3ee] mb-2">3</div>
            <div className="text-[#e4e4e7] text-center text-sm">審査後、24時間以内に結果をご連絡</div>
          </div>
        </div>
        <a
